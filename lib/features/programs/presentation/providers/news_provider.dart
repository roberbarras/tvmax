...
class NewsProvider extends ChangeNotifier {
  ...
  Future<void> fetchNews({bool loadMore = false}) async {
    ...
    if (!loadMore) {
      ...
      final localFavs = favoritesProvider.getFavoritesByCategoryId(AppConstants.newsCategoryId);
      _allNews = List.from(localFavs);
      news = List.from(_allNews);
    }
    notifyListeners();

    // Pass the News IDs here
    final result = await getPrograms(GetProgramsParams(
      page: loadMore ? _currentPage + 1 : 0,
      mainChannelId: AppConstants.newsMainChannelId,
      categoryId: AppConstants.newsCategoryId,
    ));
    ...
    result.fold(
      ...
      (r) {
        if (loadMore) {
           ...
           final existingIds = _allNews.map((p) => p.id).toSet();
           final newItems = r.where((p) => !existingIds.contains(p.id)).toList();
           _allNews.addAll(newItems);
        } else {
           final favIds = _allNews.map((p) => p.id).toSet();
  ...
  Future<void> _fetchAllPagesInBackground() async {
    ...
    while (_hasMore && _isLoadingBackground) {
      ...
      result.fold(
        (l) {
          LoggerService().debug('Background fetch news error: ${l.message}');
          _isLoadingBackground = false;
        },
        (r) {
          if (r.isEmpty) {
    ...
    LoggerService().debug('Background fetch of news completed.');
  }
